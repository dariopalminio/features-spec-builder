...
- **`story-analyze`** — añade actualización de frontmatter a `status: READY-FOR-IMPLEMENT / substatus: DONE` al finalizar sin ERROREs; si hay inconsistencias ERROR-level el estado permanece en `PLANNING/IN‑PROGRESS`; aplica tanto en modo manual como en modo Agent; confirmación final refleja el estado resultante
