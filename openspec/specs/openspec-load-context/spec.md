### Requirement: Skill openspec-load-context disponible como comando
El sistema SHALL proveer un skill invocable como `/openspec-load-context` ubicado en `.claude/skills/openspec-load-context/SKILL.md`, siguiendo las convenciones de skills del proyecto SDDF (frontmatter YAML + instrucciones Markdown).

#### Scenario: Skill existe y es invocable
- **WHEN** el usuario ejecuta `/openspec-load-context`
- **THEN** el sistema carga y ejecuta el skill desde `.claude/skills/openspec-load-context/SKILL.md`
