...
- **`story-creation` / `story-refine`**: Actualiza `status: SPECIFYING / substatus: DOING` al iniciar y `status: SPECIFIED / substatus: DONE` al finalizar la especificación.
- **`story-plan`**: Actualiza `status: PLANNING / substatus: DOING` al comenzar el pipeline de planning.
...
- **`story-implement`**: Agrega precondición — solo puede ejecutarse si `status: PLANNED` y `substatus: DONE`. Actualiza `status: IMPLEMENTING / substatus: DOING` al iniciar y `status: IMPLEMENTED / substatus: DONE` al finalizar.
...
- **Máquina de estados**: El ciclo completo queda definido formalmente como: `SPECIFYING/DOING → SPECIFIED/DONE → PLANNING/DOING → PLANNED/DONE → IMPLEMENTING/DOING → IMPLEMENTED/DONE`.
...
- `story-refine-skill`: Agrega transiciones de estado al inicio (`SPECIFYING/DOING`) y al finalizar (`SPECIFIED/DONE`).
- `story-plan-skill`: Agrega transición de estado al inicio del pipeline (`PLANNING/DOING`). Delega la transición de cierre (`PLANNED/DONE`) a `story-analyze`.
- `story-analyze-skill`: Agrega actualización de estado al finalizar (`PLANNED/DONE`).
- `story-implement-skill`: Agrega precondición de estado (`PLANNED/DONE` requerido), transición al inicio (`IMPLEMENTING/DOING`), transición al finalizar (`IMPLEMENTED/DONE`) y actualización del checklist en `release.md`.
