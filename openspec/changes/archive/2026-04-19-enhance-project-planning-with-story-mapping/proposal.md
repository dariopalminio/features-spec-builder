...
- Add a **new phase 0** to `.claude/skills/project-planning/SKILL.md`: before delegating to `project-architect`, check if `$SPECS_BASE/specs/projects/story-map.md` exists; if not, invoke the `project-story-mapping` skill (which delegates to `project-story-mapper` agent) to generate it interactively.
- Update the **delegation instruction to `project-architect`** (step 4) to also read `$SPECS_BASE/specs/projects/story-map.md` when it exists, using the backbone activities and release slices as structure for the plan's feature grouping and release definition.
