...
- [x] 1.1 Agregar sección "Verificar estado del documento de output" en `.claude/skills/ps-begin-intention/SKILL.md`: leer `$SPECS_BASE/specs/projects/project-intent.md`, detectar campo `**Estado**:` y bifurcar según valor (`IN‑PROGRESS` → retoma, `DONE` → pedir confirmación, inexistente → continuar)
- [x] 1.2 Agregar sección "Verificar estado del documento de output" en `.claude/skills/ps-discovery/SKILL.md`: leer `$SPECS_BASE/specs/projects/project.md`, aplicar misma lógica de bifurcación
- [x] 1.3 Agregar sección "Verificar estado del documento de output" en `.claude/skills/ps-planning/SKILL.md`: leer `$SPECS_BASE/specs/projects/project-plan.md`, aplicar misma lógica de bifurcación

## 2. Retoma de proyecto (project-retake)

- [x] 2.1 Agregar instrucción de retoma en el paso de delegación de `.claude/skills/ps-begin-intention/SKILL.md`: cuando el documento está en `IN‑PROGRESS`, el agente debe leer el documento existente, identificar secciones con placeholders `[...]` y continuar solo con ellas
...
- [x] 4.1 Actualizar la verificación de precondición en `.claude/skills/ps-discovery/SKILL.md`: además de verificar que `project-intent.md` existe, verificar que su `Estado` es `DONE` (no solo que el archivo existe)
...
- [x] 6.1 (**Revisión humana**) Ejecutar `/ps-begin-intention` sobre un `project-intent.md` existente en estado `IN‑PROGRESS` y verificar que el agente retoma desde las secciones incompletas sin preguntar las ya completadas
- [x] 6.2 (**Revisión humana**) Ejecutar `/ps-begin-intention` sobre un `project-intent.md` existente en estado `DONE` y verificar que el skill pide confirmación antes de sobrescribir
- [x] 6.3 (**Revisión humana**) Ejecutar `/ps-begin-intention` con un documento en `IN‑PROGRESS` en `$SPECS_BASE/specs/projects/` y verificar que el conflicto WIP es detectado y se ofrecen las dos opciones
