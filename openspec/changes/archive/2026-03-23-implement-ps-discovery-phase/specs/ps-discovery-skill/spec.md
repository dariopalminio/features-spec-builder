...
Antes de delegar al agente, el skill SHALL verificar que `$SPECS_BASE/specs/project-intent.md` existe.

#### Scenario: Input presente
- **WHEN** `$SPECS_BASE/specs/project-intent.md` existe
- **THEN** el skill continúa al siguiente paso sin interrumpir al usuario

#### Scenario: Input ausente
- **WHEN** `$SPECS_BASE/specs/project-intent.md` no existe
...
- **THEN** el skill lanza el `discovery-agent` indicando que lea `project-intent.md` y `.claude/skills/ps-discovery/templates/discovery-template.md`, conduzca la entrevista y escriba `$SPECS_BASE/specs/discovery.md`

### Requirement: Confirmación del output
Al finalizar el agente, el skill SHALL verificar que `$SPECS_BASE/specs/discovery.md` fue generado.

#### Scenario: Output generado correctamente
- **WHEN** `$SPECS_BASE/specs/discovery.md` existe tras la ejecución del agente
...
- **WHEN** `$SPECS_BASE/specs/discovery.md` no existe tras la ejecución del agente
