...
3. Si confirma: actualiza `**substatus**: IN‑PROGRESS` → `**substatus**: DONE` con Edit
...
- `DONE` → ofrece saltar la fase (ya completa) o rehacerla
- `IN‑PROGRESS` → retoma desde donde quedó
...
- [Edit del campo Estado] El orquestador usa Edit para cambiar `IN‑PROGRESS` → `DONE` en el documento; si el formato del campo cambia, el Edit puede fallar → Mitigación: usar `replace_all: false` con el string exacto `**substatus**: IN‑PROGRESS`.
