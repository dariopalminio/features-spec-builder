...
- **THEN** el skill delega la ejecución al `specifying-agent` con instrucciones para leer `discovery.md` y el template, conducir la entrevista y escribir `docs/specs/project/project.md`

#### Scenario: Output generado exitosamente
- **WHEN** el `specifying-agent` finaliza su ejecución
- **THEN** el skill verifica que `docs/specs/project/project.md` existe y confirma al usuario con el mensaje de éxito y el siguiente paso (`/ps-approve`)

#### Scenario: Output no generado
- **WHEN** el `specifying-agent` finaliza pero `docs/specs/project/project.md` no existe
