...
- **WHEN** el usuario ejecuta `/ps-specifying` y `docs/specs/projects/discovery.md` no existe
...
- **THEN** el skill delega la ejecución al `specifying-agent` con instrucciones para leer `discovery.md` y el template, conducir la entrevista y escribir `docs/specs/projects/project.md`

#### Scenario: Output generado exitosamente
- **WHEN** el `specifying-agent` finaliza su ejecución
- **THEN** el skill verifica que `docs/specs/projects/project.md` existe y confirma al usuario con el mensaje de éxito y el siguiente paso (`/ps-approve`)

#### Scenario: Output no generado
- **WHEN** el `specifying-agent` finaliza pero `docs/specs/projects/project.md` no existe
