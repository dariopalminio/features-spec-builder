...
- [x] 2.3 Agregar lógica de validación de estado: si `Doing` → preguntar al usuario si confirma avanzar; si confirma → usar `Edit` para cambiar `**Estado**` a `Ready`; si rechaza → detener y remitir a `/ps-funnel`
...
- [x] 2.5 Agregar instrucciones de escritura: crear `docs/specs/project-intent.md` con estructura del template, contenido recopilado y metadatos (fecha, agente, `**Estado**: Doing`)

## 3. Skill ps-draft

- [x] 3.1 Reescribir `.claude/skills/ps-draft/SKILL.md`: verificar existencia de `docs/specs/initial-prompt.md`; si no existe → error con sugerencia de `/ps-funnel`
...
- [x] 3.4 Agregar confirmación final: verificar que `docs/specs/project-intent.md` existe y confirmar al usuario con la ruta y el siguiente paso (`/ps-discover`)

## 4. Validación Manual

- [ ] 4.1 [Validación humana] Ejecutar `/ps-draft` con `initial-prompt.md` en estado `Doing` y verificar que el agente pregunta si se confirma avanzar y actualiza el campo a `Ready`
- [ ] 4.2 [Validación humana] Ejecutar `/ps-draft` con `initial-prompt.md` en estado `Ready` y verificar el flujo completo de preguntas y el documento generado
- [ ] 4.3 [Validación humana] Revisar `docs/specs/project-intent.md`: secciones completas, contenido inferido marcado, metadatos correctos
