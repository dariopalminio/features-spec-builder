...
- El skill lee la sección "## Propuesta de Releases" de `$SPECS_BASE/specs/projects/project-plan.md` y genera un archivo `release-[ID]-[Nombre].md` por cada release encontrado
- Cada archivo generado sigue exactamente la estructura de `$SPECS_BASE/specs/templates/release-spec-template.md`, populando las secciones obligatorias (frontmatter, Descripción, Features, Flujos Críticos) con los datos del plan
- Las secciones opcionales del template se incluyen con placeholder cuando no hay datos disponibles
- Los archivos se guardan en `$SPECS_BASE/specs/releases/` (creando el directorio si no existe)
...
- **Directorio de salida:** `$SPECS_BASE/specs/releases/` (creado por el skill en runtime)
- **Dependencias de lectura:** `$SPECS_BASE/specs/projects/project-plan.md`, `$SPECS_BASE/specs/templates/release-spec-template.md`
