...
The `/ps-plan` skill SHALL verify that `$SPECS_BASE/specs/projects/clarifications.md` exists before proceeding. If absent, it SHALL inform the user and halt.

#### Scenario: clarifications.md present
- **WHEN** `$SPECS_BASE/specs/projects/clarifications.md` exists
- **THEN** the skill continues to template verification

#### Scenario: clarifications.md absent
- **WHEN** `$SPECS_BASE/specs/projects/clarifications.md` does not exist
...
The skill SHALL invoke the `planning-agent` with an instruction to read all input documents, read the template, and produce `$SPECS_BASE/specs/projects/project-plan.md`.
...
After the planning-agent finishes, the skill SHALL verify that `$SPECS_BASE/specs/projects/project-plan.md` was created and inform the user.
