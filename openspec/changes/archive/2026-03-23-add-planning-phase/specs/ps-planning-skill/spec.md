...
The `/ps-plan` skill SHALL verify that `docs/specs/project/clarifications.md` exists before proceeding. If absent, it SHALL inform the user and halt.

#### Scenario: clarifications.md present
- **WHEN** `docs/specs/project/clarifications.md` exists
- **THEN** the skill continues to template verification

#### Scenario: clarifications.md absent
- **WHEN** `docs/specs/project/clarifications.md` does not exist
...
The skill SHALL invoke the `planning-agent` with an instruction to read all input documents, read the template, and produce `docs/specs/project/project-plan.md`.
...
After the planning-agent finishes, the skill SHALL verify that `docs/specs/project/project-plan.md` was created and inform the user.
