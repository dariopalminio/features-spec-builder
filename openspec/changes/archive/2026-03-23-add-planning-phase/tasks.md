...
- [x] 3.2 Add Step 1 to skill: verify `$SPECS_BASE/specs/projects/clarifications.md` exists — halt with error and `/ps-approval` instruction if missing
...
- [x] 3.4 Add Step 3 to skill: invoke `planning-agent` with instruction to read all input documents, read template, and write `$SPECS_BASE/specs/projects/project-plan.md`
- [x] 3.5 Add Step 4 to skill: verify `$SPECS_BASE/specs/projects/project-plan.md` exists after agent completes — show success message and next-step prompt, or error with re-run suggestion
