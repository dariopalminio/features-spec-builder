...
El skill SHALL escanear el directorio `$SPECS_BASE/specs/releases/` y recopilar todos los archivos con extensión `.md` para procesarlos. Si el directorio no existe o no contiene archivos `.md`, el skill MUST terminar con un mensaje descriptivo sin generar ningún archivo.

#### Scenario: Directorio con múltiples releases
- **WHEN** `$SPECS_BASE/specs/releases/` contiene tres o más archivos `.md`
- **THEN** el skill identifica todos los archivos como candidatos a procesar

#### Scenario: Directorio vacío o inexistente
- **WHEN** `$SPECS_BASE/specs/releases/` no existe o no contiene archivos `.md`
...
- **WHEN** el skill detecta que 5 de 20 historias a generar ya existen en `$SPECS_BASE/specs/stories/`
...
- **WHEN** ninguna de las historias a generar ya existe en `$SPECS_BASE/specs/stories/`
