...
Toma una historia grande, épica o feature demasiado amplio y lo divide en historias más pequeñas e independientes. Cada historia resultante sigue **estrictamente** el template `assets/story-gherkin-template.md`.
...
**Actualizar frontmatter** de cada `story.md` con `id: FEAT-{NNN}` y `slug: FEAT-{NNN}-{slug}`.
