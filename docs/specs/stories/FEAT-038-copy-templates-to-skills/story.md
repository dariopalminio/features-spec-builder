...
type: story
id: FEAT-038
slug: FEAT-038-copy-templates-to-skills
...
**Quiero** copiar todos los templates de `$SPECS_BASE/specs/templates/` al directorio `templates/` del skill que los usa
**Para** que cada skill sea aut�nomo `$SPECS_BASE/specs/s/` solo contenga documentos generados (salida)
...
### Escenario alternativo � Template referenciado por m�ltiples skills
```gherkin
Dado que un template es referenciado por m�s de un skill
Cuando se decide d�nde ubicarlo
Entonces se copia al skill que es su "due�o principal" (el que lo define o usa m�s)
  Y cada skill que lo necesita recibe su propia copia local en su carpeta "templates/"
```

### Requerimiento: Excluir templates no usados por ning�n skill activo
Los templates de `others_references/` (openspec-*.md) quedan excluidos de este movimiento si no corresponden a ning�n skill activo; se decide su destino por separado.
...
* Trazabilidad: cada template copiado debe poder asociarse a su skill due�o sin ambig�edad
* Compatibilidad: los skills ya migrados (`project-begin`, `project-planning`) no se modifican

## ?? Notas / contexto adicional

Templates actuales en `$SPECS_BASE/specs/templates/` 

La carpeta `$SPECS_BASE/specs/templates/others_references/` (openspec design, proposal, spec, tasks) queda fuera del scope de esta historia; se tratar� en una historia separada si aplica.
