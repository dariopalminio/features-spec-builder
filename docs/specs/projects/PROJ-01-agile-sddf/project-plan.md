...
- [ ] **FEAT-006: Creación de Historias de Usuario** — El sistema genera historias completas (en fase SPECIFYING de story) en formato Como/Quiero/Para con criterios de aceptación Gherkin (mínimo 1 escenario principal y 1 alternativo), guardando el resultado en `story-{slug}.md`. _(deps: —)_
- [ ] **FEAT-007: Evaluación de Calidad con Rúbrica FINVEST** — El sistema evalúa historias de usuario (en fase SPECIFYING de story) aplicando la rúbrica FINVEST (Formato + INVEST) con scores Likert 1-5 por dimensión y produce una decisión APROBADA / REFINAR / RECHAZAR / DIVIDIR. _(deps: FEAT-006)_
...
- [ ] **Skill para Tasking:** crear un skill generador de tareas para planificar (en fase PLANNING de story) tareas de implementación llamado `story-tasking` (algo equivalente al comando speckit.task de speckit o a el tasks.md de openspec que se crea cuando se llama a proposal).
- [ ] **Comando de análisis transversal:** crear un comando llamado `/story-analyze` (similar a /speckit.analyze) que se ejecute idealmente después de `/story-tasking` y antes de `/story-implement` (en fase  PLANNING de story). Su objetivo es identificar inconsistencias y omisiones entre los artefactos clave (story.md, design.md, tasks.md) y verificar su alineación con el release y el contexto del proyecto.
- [ ] **Orquestación del plan:** crear un skill de orquestación llamado `story-plan` (en la fase de PLANNING de story) que ejecute los skills anteriores en secuencia (story-design --> story-tasking --> story-analyze) para una historia objetivo `story.md`. Al ejecutar `/story-plan`, debe generar primero el `design.md`, luego el `tasks.md`, y finalmente `/story-analyze`.
- [ ] **Skill de implementación (El programador autónomo):** crear un skill llamado `story-implement` (similar a /speckit.implement) que se encargue de la fase de implementación de la historia `story.md` (en fase IMPLEMENTING de story). Se encarga de ejecutar las tareas y generar el código automáticamente de historias que están esperando ser implementadas (fase READY-FOR-IMPLEMENT).
- [ ] **Status management on workflow:** mejorar todos los skills de story para que actualicen el estado de la historia en el frontmatter de story.md, reflejando el progreso a través de los estados: "specify", "design", "tasking", "analyzing", "implementing".
- [ ] **Skill de Code Review:** La capacidad /story-code-review (en fase post IMPLEMENTING de story) implementa una revisión de código automatizada mediante un equipo de tres agentes especializados (Calidad, Requisitos e Integración + un árbitro), garantizando que la implementación de una historia de usuario cumple con los criterios de aceptación (Gherkin), los estándares técnicos y la coherencia arquitectónica.
